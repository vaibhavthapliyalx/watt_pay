...
const GenerateInvoiceScreen = ({onGeneratePressed, isDisplayed}: IProps) => {
  ...
  const handleGenerateInvoice = async () => {
    console.log(`here:${ invoiceData.currentUnits.length}`)
    // Prompt if data entered is null or undefined.
    if (invoiceData === undefined || invoiceData.currentUnits.length === 0)
    {
      Alert.alert('Invalid Input !',`Please enter some data in the fields provided.`, [
        { text: 'Continue'},]);
    } else {
